...
### 1.10 Live Activity Log (Terminal Console)
- A real-time dark-themed console at the bottom of the workspace
- Each step of the pipeline (parsing → cohort fetch → AI generation → filtering → dispatch) is logged with:
  - Timestamp
  - Message
  - Status icon (loading spinner / info / success / error)
