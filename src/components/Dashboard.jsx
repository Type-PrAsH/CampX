      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-500 mt-1">
            Real-time performance overview of your email campaigns.
          </p>
        </div>
        <button
        ...
        {realDashboardMetrics.map((metric, idx) => (
          ...
          >
            ...
            <div className="flex items-baseline gap-2 mt-2">
              ...
              {metric.change && (
                <span
                  className={`text-xs font-semibold flex items-center gap-0.5 ${metric.trend === "up" ? "text-emerald-600" : "text-red-600"}`}
                >
                  {metric.trend === "up" ? (
            ...
            <ResponsiveContainer width="100%" height={256}>
              <BarChart data={chartData.clickRateBySegment} layout="vertical">
                ...
                <Bar
                  ...
                  barSize={20}
                >
                  {chartData.clickRateBySegment.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={index === 3 ? "#6366F1" : "#c7d2fe"}
                    />
                  ))}
                </Bar>
              </BarChart>
